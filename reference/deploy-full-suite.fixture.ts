// SOURCE: https://github.com/ERC-3643/ERC-3643/blob/main/test/fixtures/deploy-full-suite.fixture.ts
// This is the official T-REX test fixture showing the complete deployment sequence.
// Downloaded for reference during VerdiChain implementation.

import { ethers } from 'hardhat';
import { BigNumber } from 'ethers';
import OnchainID from '@onchain-id/solidity';

// Helper function referenced but not shown in fixture - deploys identity via proxy
// async function deployIdentityProxy(implementationAuthorityAddress, ownerAddress, deployer) { ... }
...
export async function deployFullSuiteFixture() {
  ...
  // ============================================================
  // STEP 1: Deploy all implementation contracts (logic contracts)
  // ============================================================
  ...
  // ============================================================
  // STEP 2: Deploy OnchainID implementation + authority + factory
  // ============================================================
  ...
  // ============================================================
  // STEP 3: Deploy TREXImplementationAuthority with version
  // ============================================================
  ...
  // ============================================================
  // STEP 4: Deploy TREXFactory and link to identity factory
  // ============================================================
  ...
  // IMPORTANT: Must add the TREX factory to the identity factory
  await identityFactory.connect(deployer).addTokenFactory(trexFactory.address);

  // ============================================================
  // STEP 5: Deploy proxied contracts via implementation authority
  // (NOTE: In our case, we use deployTREXSuite instead of manual proxy deployment)
  // ============================================================
  ...
  // ============================================================
  // STEP 7: Bind identity registry storage and add agents
  // ============================================================
  ...
  // ============================================================
  // STEP 8: Set up claim topics
  // ============================================================
  const claimTopics = [ethers.utils.id('CLAIM_TOPIC')];
  await claimTopicsRegistry.connect(deployer).addClaimTopic(claimTopics[0]);

  // ============================================================
  // STEP 9: Deploy ClaimIssuer and add signing key
  // ============================================================
  ...
  // ============================================================
  // STEP 10: Deploy identities for users
  // ============================================================
  ...
  // ============================================================
  // STEP 11: Register identities and add agents
  // ============================================================
  ...
  // Country codes: 42 and 666 are test values
  ...
  // ============================================================
  // STEP 12: Issue claims to identities
  // ============================================================
  const claimForAlice = {
  ...
  // ============================================================
  // STEP 13: Mint tokens and unpause
  // ============================================================
  ...
  return {
    accounts: {
      deployer, tokenIssuer, tokenAgent, tokenAdmin, claimIssuer,
      claimIssuerSigningKey, aliceActionKey,
      aliceWallet, bobWallet, charlieWallet, davidWallet, anotherWallet,
    },
    identities: { aliceIdentity, bobIdentity, charlieIdentity },
    suite: {
      claimIssuerContract, claimTopicsRegistry, trustedIssuersRegistry,
      identityRegistryStorage, defaultCompliance, identityRegistry, tokenOID, token,
    },
    authorities: { trexImplementationAuthority, identityImplementationAuthority },
    factories: { trexFactory, identityFactory },
    implementations: {
      identityImplementation, claimTopicsRegistryImplementation,
      trustedIssuersRegistryImplementation, identityRegistryStorageImplementation,
      identityRegistryImplementation, modularComplianceImplementation, tokenImplementation,
    },
  };
}
