...
export default function ComplianceMonitor() {
  ...
  const displayTotal = useCountUp(events.length);
  const displayApprovals = useCountUp(approvals);
  const displayRestrictions = useCountUp(restrictions);
      ...
      <div {...entranceProps(1, "bg-surface-2 border-y border-border full-bleed")}>
        <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-border">
          <div className="py-5 sm:py-6 sm:pr-6">
            ...
            <p className="font-display text-3xl sm:text-4xl lg:text-5xl text-text">{displayTotal}</p>
          </div>
          <div className="py-5 sm:py-6 sm:px-6">
            <p className="stat-label mb-2">Approvals</p>
            <p className="font-display text-3xl sm:text-5xl text-bond-green">{displayApprovals}</p>
          </div>
          <div className="py-5 sm:py-6 sm:pl-6">
            <p className="stat-label mb-2">Restrictions</p>
            <p className="font-display text-3xl sm:text-5xl text-bond-red">{displayRestrictions}</p>
          </div>
        </div>
      </div>
      ...
      {coupons && coupons.length > 0 && (
        <section {...entranceProps(2)}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {coupons.map((c, idx) => (
              <div key={c.id} {...entranceProps(idx + 3, "card-static text-xs")}>
      ...
      <div role="tablist" aria-label="Event source" {...entranceProps(3, "flex gap-1 bg-surface-2 rounded-lg p-1 w-fit")}>
        <button
          ...
          className={`px-4 py-2 text-sm rounded-md transition-all duration-200 ${
            tab === "onchain" ? "bg-surface-3 text-text font-medium shadow-sm" : "text-text-muted hover:text-text"
          }`}
        >
          On-Chain Events
        </button>
        <button
          ...
          className={`px-4 py-2 text-sm rounded-md transition-all duration-200 ${
            tab === "guardian" ? "bg-surface-3 text-text font-medium shadow-sm" : "text-text-muted hover:text-text"
          }`}
        >
