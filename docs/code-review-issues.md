...
**Works for SDK-created tokens**: eUSD was created via Hedera SDK in `middleware/src/hts-setup.ts`, NOT via a smart contract. Per HIP-218 and HIP-376, the ERC-20 facade applies to ALL HTS tokens regardless of how they were created. Confirmed: `balanceOf`, `transfer`, `approve`, `transferFrom` all work on SDK-created tokens.
...
**ABI is already generated**: `countryRestrictModuleAbi` is exported from `@coppice/abi` (configured in `packages/abi/wagmi.config.ts` line 18).
...
- **Files**: `middleware/src/event-logger.ts:55-57`
...
### 9. Contract address duplication between @coppice/abi and env vars
- **Files**: `packages/abi/src/generated.ts`, `frontend/lib/constants.ts:15-17`
- **Issue**: Testnet addresses baked into `@coppice/abi` package (generated by wagmi CLI) AND overridable by `NEXT_PUBLIC_*` env vars. Two sources of truth — redeployment requires updating both.
- **Note**: Not blocking. Document that `@coppice/abi` addresses are the source of truth and env vars are overrides for non-standard deployments.
...
- **Files**: `middleware/src/event-logger.ts:208`
