...
- JSON payloads kept under 1KB using abbreviated addresses. Largest payload (TRANSFER with full addresses) is only ~244 bytes — well under 1024. TODO: Remove address abbreviation in event-logger.ts so HCS messages contain full addresses.
- Uses ethers.js v6 event listener pattern (TODO: event-logger.ts and purchase-api.ts still use ethers — port to viem for consistency)
...
- **Actual:** Browser cannot submit HCS messages directly (requires Hedera SDK private key signing). The IssuerDashboard now calls the `/api/allocate` backend endpoint which submits to HCS via the middleware. This is fully functional.
...
- **Actual:** Manual polish pass covering all pages and components (impeccable skills deferred to post-Next.js port):
...
- **All original plan tasks (1-15) completed**
- **Plans 1-3 (low-hanging fruit, contracts viem, frontend viem) completed and merged**
